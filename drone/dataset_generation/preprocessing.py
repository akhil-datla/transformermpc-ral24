...
from dynamics.quadrotor import compute_constraint_to_go, compute_reward_to_go, obs_positions, obs_radii
from optimization.quad_scenario import dataset_scenario

parser = argparse.ArgumentParser(description='transformer-quad')
...
data_cvx = np.load(args.data_dir + '/dataset-quad-v05-cvx.npz')
data_param = np.load(args.data_dir + '/dataset-quad-v05-param.npz') 
...
torch_ctgs_scp = torch.from_numpy(compute_constraint_to_go(states_scp, obs_positions, obs_radii))
torch.save(torch_ctgs_scp, args.data_dir_torch + '/torch_ctgs_scp.pth')

torch_ctgs_cvx = torch.from_numpy(compute_constraint_to_go(states_cvx, obs_positions, obs_radii))
